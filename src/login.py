...
def load_preset_from_env():
    ...
    keys = ['SESSDATA', 'buvid3', 'buvid4', 'bili_jct', 'ac_time_value', 'DedeUserID', 'sessdata', 'dedeuserid']
    cookies = {}
    for k in keys:
        v = os.environ.get(k)
        if v:
...
def check_login_with_cookies(cookies_dict) -> bool:
    ...
    try:
        ...
        data = resp.json()
        if isinstance(data, dict) and data.get('code') == 0 and data.get('data', {}).get('mid'):
            return True
